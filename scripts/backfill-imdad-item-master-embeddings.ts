/**
 * Backfill embeddings for ImdadItemMaster rows that have "embeddingVec" = NULL.
 *
 * Usage:
 *   npx tsx scripts/backfill-imdad-item-master-embeddings.ts [--tenant <uuid>] [--dry-run]
 *
 * Options:
 ...
 *
 * Behaviour:
 *   Cursor-paginated by createdAt; batch size 25 per OpenAI call (matches
 *   PolicyChunk pattern); 200 ms sleep between batches.
 ...
 *   ~50,000 items × ~30 tokens/item → ~1.5M tokens → ~$0.20 total.
 */
...
import { embedImdadItemMaster } from '../lib/embeddings/writers/imdadItemMaster';
...
async function main(): Promise<void> {
  ...
  console.log('\n📦  ImdadItemMaster Embedding Backfill (Phase 7.2)');
  ...
  const totalCount: number = await prisma
    .$queryRawUnsafe(
      `SELECT COUNT(*) AS count FROM "imdad_item_masters" WHERE "isDeleted" = false${tenantArg ? ' AND "tenantId" = $1::uuid' : ''}`,
      ...(tenantArg ? [tenantArg] : []),
    )
  ...
  const embeddedCount: number = await prisma
    .$queryRawUnsafe(
      `SELECT COUNT(*) AS count FROM "imdad_item_masters" WHERE "embeddingVec" IS NOT NULL AND "isDeleted" = false${tenantArg ? ' AND "tenantId" = $1::uuid' : ''}`,
      ...(tenantArg ? [tenantArg] : []),
    )
    .then((r: { count: bigint }[]) => Number(r[0]?.count ?? 0));
  ...
  if (isDryRun) {
    const estimatedTokens = pendingCount * 30; // ~30 tokens / item avg
  ...
  while (true) {
    const batch: RowId[] = await prisma.$queryRawUnsafe(
      `SELECT "id" FROM "imdad_item_masters"
    ...
    for (const row of batch) {
      try {
        const outcome = await embedImdadItemMaster(row.id, { prismaClient: prisma });
        if (outcome.skipped) {
          skipped++;
        } else {
