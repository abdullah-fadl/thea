/**
 * Backfill embeddings for PolicyChunk rows that have "embeddingVec" = NULL.
 *
 * Usage:
 *   npx tsx scripts/backfill-policy-chunk-embeddings.ts [--tenant <uuid>] [--dry-run]
 *
 * Options:
 ...
 *
 * Behaviour:
 *   Cursor-paginated by createdAt; batch size 25 per OpenAI call (smaller
 *   than the department backfill because policy chunks are longer texts);
 *   200 ms sleep between batches.
 *   Idempotent — rows where "embeddingVec" IS NOT NULL are skipped.
 *   Safe to interrupt and resume — re-running picks up the remaining NULLs.
 *
 * Cost estimate at $0.13 / 1M tokens (text-embedding-3-large @ 1536 dims):
 *   ~10,000 chunks × ~200 tokens/chunk → ~2M tokens → ~$0.26 total.
 */
...
import { embedPolicyChunk } from '../lib/embeddings/writers/policyChunk';
...
async function main(): Promise<void> {
  ...
  console.log('\n📦  PolicyChunk Embedding Backfill (Phase 7.1)');
  ...
  const totalCount: number = await prisma
    .$queryRawUnsafe(
      `SELECT COUNT(*) AS count FROM "policy_chunks"${tenantArg ? ' WHERE "tenantId" = $1::uuid' : ''}`,
      ...(tenantArg ? [tenantArg] : []),
    )
  ...
  const embeddedCount: number = await prisma
    .$queryRawUnsafe(
      `SELECT COUNT(*) AS count FROM "policy_chunks" WHERE "embeddingVec" IS NOT NULL${tenantArg ? ' AND "tenantId" = $1::uuid' : ''}`,
      ...(tenantArg ? [tenantArg] : []),
    )
    .then((r: { count: bigint }[]) => Number(r[0]?.count ?? 0));
  ...
  if (isDryRun) {
    const estimatedTokens = pendingCount * 200; // ~200 tokens / chunk avg
  ...
  // Cursor by createdAt for resumable pagination. We re-query each batch from
  // scratch (always picking the next NULLs) so progress survives interruption.
  while (true) {
    const batch: RowId[] = await prisma.$queryRawUnsafe(
      `SELECT "id" FROM "policy_chunks"
       WHERE "embeddingVec" IS NULL${tenantArg ? ' AND "tenantId" = $1::uuid' : ''}
       ORDER BY "createdAt"
       LIMIT ${BATCH_SIZE}`,
      ...(tenantArg ? [tenantArg] : []),
    );
    ...
    for (const row of batch) {
      try {
        const outcome = await embedPolicyChunk(row.id, { prismaClient: prisma });
        if (outcome.skipped) {
          skipped++;
        } else {
