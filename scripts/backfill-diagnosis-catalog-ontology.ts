/**
 * Phase 7.3 — Backfill OntologyMapping rows for DiagnosisCatalog → ICD-10-AM.
 *
 * Mirror of scripts/backfill-formulary-drug-ontology.ts for the
 * DiagnosisCatalog.icd10 column.
 *
 * Usage:
 *   npx tsx scripts/backfill-diagnosis-catalog-ontology.ts [--tenant <uuid>] [--dry-run]
 *
 * Environment:
 *   Reads MIGRATION_URL / DIRECT_URL / DATABASE_URL from .env.local.
 *   Forces THEA_FF_ONTOLOGY_ENABLED=true for this process.
 *
 * Behavior:
 *   - Cursor-paginated SELECT id ORDER BY id, batch 100, 50 ms courtesy sleep.
 *   - No external API calls.
 *   - Idempotent: a re-run produces zero new mappings if everything is wired.
 *
 * Report shape (final stdout JSON line):
...
async function main(): Promise<void> {
  ...
  console.log('\n📦  DiagnosisCatalog → ICD-10-AM Ontology Backfill');
  ...
  const totalRows: number = await prisma.$queryRawUnsafe(
    `SELECT COUNT(*) AS count FROM diagnosis_catalog${tenantArg ? ' WHERE "tenantId" = $1::uuid' : ''}`,
    ...(tenantArg ? [tenantArg] : []),
  ).then((r: { count: bigint }[]) => Number(r[0]?.count ?? 0));

  const eligibleRows: number = await prisma.$queryRawUnsafe(
    `SELECT COUNT(*) AS count FROM diagnosis_catalog WHERE icd10 IS NOT NULL AND icd10 <> ''${tenantArg ? ' AND "tenantId" = $1::uuid' : ''}`,
    ...(tenantArg ? [tenantArg] : []),
  ).then((r: { count: bigint }[]) => Number(r[0]?.count ?? 0));

  console.log(`  Total diagnoses    : ${totalRows}`);
  console.log(`  With icd10 code    : ${eligibleRows}`);
  ...
  while (true) {
    ...
    const batch: RowId[] = await prisma.$queryRawUnsafe(
      `SELECT id FROM diagnosis_catalog WHERE icd10 IS NOT NULL AND icd10 <> ''${
    ...
    for (const row of batch) {
      try {
        const result = await mapDiagnosisCatalogToIcd10(row.id);
