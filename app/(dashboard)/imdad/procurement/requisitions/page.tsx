...
function statusBadge(status: string) {
  const map: Record<string, string> = {
    ...
    CANCELLED: 'bg-[#E8A317]/10 text-[#C4960C] dark:bg-[#E8A317]/20 dark:text-[#E8A317]',
  };
...
export default function RequisitionsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/imdad/procurement/requisitions?${params}`);
      if (res.ok) {
        const data: RequisitionsResponse = await res.json();
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {tr('طلبات الشراء', 'Purchase Requisitions')}
          </h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {tr('إدارة ومتابعة طلبات الشراء', 'Manage and track purchase requisitions')}
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={tr('بحث برقم الطلب...', 'Search by PR number...')}
            value={search}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                  {tr('لا توجد طلبات شراء', 'No purchase requisitions found')}
                </td>
              </tr>
            ) : (
              requisitions.map((r) => (
                <tr key={r._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-[#D4A017] dark:text-[#E8A317]">{r.prNumber}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                    {language === 'ar' && r.requestedByNameAr ? r.requestedByNameAr : r.requestedBy}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                    {language === 'ar' && r.departmentAr ? r.departmentAr : r.department}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                    {r.totalAmount?.toLocaleString(language === 'ar' ? 'ar-SA' : 'en-SA', { style: 'currency', currency: r.currency || 'SAR' })}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${statusBadge(r.status)}`}>
                      {tr(statusTr[r.status]?.[0] ?? r.status, statusTr[r.status]?.[1] ?? r.status)}
                    </span>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                    {new Date(r.createdAt).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-SA')}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
