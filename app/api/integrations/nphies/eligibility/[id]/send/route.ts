// Phase 8.1.4 — Trigger sendEligibilityCheck for an existing eligibility log.
// POST /api/integrations/nphies/eligibility/[id]/send
  ...
  withErrorHandler(async (_req: NextRequest, { tenantId }, params) => {
    ...
    if (!id) {
      return NextResponse.json(
        operationOutcomeError('error', 'invalid', 'Missing eligibility log id'),
        { status: 400, headers: FHIR_HEADERS },
      );
    }
    try {
      const result = await sendEligibilityCheck({ eligibilityRequestId: id, tenantId });
      return NextResponse.json(result.bundle, {
        status: result.httpStatus,
        headers: { ...FHIR_HEADERS, 'X-Correlation-Id': result.correlationId },
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'NPHIES eligibility send failed';
