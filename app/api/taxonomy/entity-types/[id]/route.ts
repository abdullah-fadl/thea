...
export const DELETE = withErrorHandler(async (
  ...
  { params }: { params: { id: string } }
    ...
    const { id } = params;
    if (!id) return NextResponse.json({ error: 'Entity type ID is required' }, { status: 400 });

    const tenant = await prisma.tenant.findFirst({ where: tenantWhere(tenantId) });
    if (!tenant) return NextResponse.json({ error: 'Tenant not found' }, { status: 404 });

    const existing = await prisma.taxonomyEntityType.findFirst({
      where: { id, tenantId: tenant.id },
    });
    if (!existing) return NextResponse.json({ error: 'Entity type not found' }, { status: 404 });

    await prisma.taxonomyEntityType.update({
