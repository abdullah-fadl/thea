...
const createRiskDomainSchema = z.object({
  name: z.string().min(1),
...
export const GET = withErrorHandler(async (request: NextRequest) => {
    ...
    const riskDomains = await prisma.taxonomyRiskDomain.findMany({
...
export const POST = withErrorHandler(async (request: NextRequest) => {
    ...
    const v = validateBody(body, createRiskDomainSchema);
    ...
    const nameConditions: any[] = [{ name: { equals: name.trim(), mode: 'insensitive' } }];
    if (code) nameConditions.push({ code: { equals: code, mode: 'insensitive' } });

    const existing = await prisma.taxonomyRiskDomain.findFirst({
      where: { tenantId: tenant.id, isActive: true, OR: nameConditions },
    });
    if (existing) {
      return NextResponse.json({ error: 'Risk domain with this name or code already exists' }, { status: 409 });
    }

    const riskDomain = await prisma.taxonomyRiskDomain.create({
      data: {
        tenantId: tenant.id,
        name: name.trim(),
