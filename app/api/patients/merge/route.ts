...
export const POST = withAuthTenant(
  ...
  const cascadeResults = await prisma.$transaction(async (tx) => {
    ...
    // oncologyProtocol.patientId and chemoCycle.patientId reference OncologyPatient.id (not PatientMaster.id)
    // They follow the OncologyPatient record which itself gets reassigned above — no direct cascade needed
