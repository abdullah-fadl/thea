...
export const POST = withAuthTenant(
  ...
  if (nextState === 'COMPLETED') {
    ...
    // Emit encounter.closed@v1 — fires on the COMPLETED transition only.
    // currentState !== 'COMPLETED' is guaranteed by the early no-op return above.
    try {
      await emit({
      ...
      logger.error('events.emit_failed', { category: 'opd', eventName: 'encounter.closed', error: e });
    }
