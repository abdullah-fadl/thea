...
export const GET = withAuthTenant(
  withErrorHandler(async (req, { tenantId, userId, role }, params) => {
    try {
      ...
      void shadowEvaluate({ legacyDecision: 'allow', action: 'View', principal: { id: userId, type: 'Thea::User', attrs: { tenantId, role: role ?? '', hospitalId: '' } }, resource: { id: policyId, type: 'Thea::SamPolicy', attrs: { tenantId, status: 'published', scope: 'enterprise' } } });
...
export const POST = withAuthTenant(
  withErrorHandler(async (req, { tenantId, userId, user, role }, params) => {
    try {
      const resolvedParams = params instanceof Promise ? await params : params;
      const policyId = String((resolvedParams as Record<string, string>)?.policyId || '').trim();
      if (!policyId) {
      ...
      void shadowEvaluate({ legacyDecision: 'allow', action: 'Update', principal: { id: userId, type: 'Thea::User', attrs: { tenantId, role: role ?? '', hospitalId: '' } }, resource: { id: policyId, type: 'Thea::SamPolicy', attrs: { tenantId, status: 'published', scope: 'enterprise' } } });
      ...
      // Emit policy.acknowledged@v1 — best-effort, never breaks the response.
      try {
        await emit({
          ...
          aggregateId: acknowledgment.id,
          payload: {
            acknowledgmentId: acknowledgment.id,
            tenantId,
            policyId,
            userId,
            version: v.data.version ?? null,
            acknowledgedAt: (acknowledgment.acknowledgedAt instanceof Date ? acknowledgment.acknowledgedAt : new Date()).toISOString(),
          },
        });
      } catch (e) {
        logger.error('events.emit_failed', { category: 'sam', eventName: 'policy.acknowledged', error: e });
      }
