...
import { invalidateSpecialties } from '@/lib/cache/invalidation';
...
export const GET = withAuthTenant(async (req: NextRequest, { tenantId, userId, user }) => {
  ...
  const items = canCache
    ? await cached(CacheKeys.specialtyList(tenantId), fetchSpecialties, CacheTTL.DEPARTMENTS)
...
export const POST = withAuthTenant(async (req: NextRequest, { tenantId, userId, user }) => {
  ...
  const postSchema = z.object({ name: z.string().min(1), code: z.string().optional(), clientRequestId: z.string().optional() }).passthrough();
  ...
  return withIdempotency({
    ...
    handler: async () => {
      ...
      await invalidateSpecialties(tenantId);
      return result;
    },
  });
