...
export const GET = withAuthTenant(
  async (req: NextRequest, { tenantId }: { tenantId: string }) => {
    try {
      ...
      logger.error('[ADCTRANSACTION GET] Failed', { category: 'api', error: e instanceof Error ? e : undefined });
...
export const POST = withAuthTenant(
  async (req: NextRequest, { tenantId, userId }: { tenantId: string; userId: string }) => {
    try {
      ...
      const item = await prisma.adcTransaction.create({
        data: { tenantId, ...body, createdByUserId: userId },
      });
      return NextResponse.json({ item }, { status: 201 });
    } catch (e) {
      logger.error('[ADCTRANSACTION POST] Failed', { category: 'api', error: e instanceof Error ? e : undefined });
