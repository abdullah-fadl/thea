...
export const POST = withAuthTenant(
  async (req, { tenantId, userId, role }) => {
    try {
      ...
      void shadowEvaluate({ legacyDecision: 'allow', action: 'Create', principal: { id: userId, type: 'Thea::User', attrs: { tenantId, role: role ?? '', hospitalId: '' } }, resource: { id: po.id, type: 'Thea::ImdadPurchaseOrder', attrs: { tenantId, organizationId: String(parsed.organizationId ?? ''), status: String((po as any)?.status ?? 'DRAFT'), amount: Math.round(Number((po as any)?.totalAmount ?? totalAmount ?? 0)) } } });

      // Emit purchase_order.created@v1 — best-effort, never breaks the response.
      try {
        await emit({
          ...
          payload: {
            ...
            organizationId: parsed.organizationId,
            vendorId: parsed.vendorId,
            status: (po as any).status ?? 'DRAFT',
            currency: parsed.currency,
            createdAt: ((po as any).createdAt instanceof Date ? (po as any).createdAt : new Date()).toISOString(),
          },
        });
      } catch (e) {
        logger.error('events.emit_failed', { category: 'imdad', eventName: 'purchase_order.created', error: e });
      }
