...
export const POST = withAuthTenant(
  async (req, { tenantId, userId, role }) => {
    try {
      ...
      // Emit goods_received@v1 — best-effort, never breaks the response.
      try {
        await emit({
          ...
          payload: {
            ...
            status: (grn as any).status ?? 'DRAFT',
            receivedAt: ((grn as any).receivedAt instanceof Date ? (grn as any).receivedAt : new Date()).toISOString(),
          },
        });
      } catch (e) {
        logger.error('events.emit_failed', { category: 'imdad', eventName: 'goods_received', error: e });
      }
