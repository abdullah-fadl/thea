...
export const POST = withAuthTenant(
  ...
  // Emit lab.result.posted@v1 — best-effort, only on the finalize/post path.
  // Skipped for IN_PROGRESS partial saves; the event represents the moment
  // the result becomes consumable downstream. Payload is IDs + status only —
  // no parameter values, abnormal flags, or test display names are emitted.
  if (status === 'COMPLETED' || status === 'VERIFIED' || status === 'RESULTED') {
    try {
      await emit({
      ...
      logger.error('events.emit_failed', { category: 'clinical', eventName: 'lab.result.posted', error: e });
    }
  }
