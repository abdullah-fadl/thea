...
import { rejectClaimSchema } from '@/lib/validation/billing.schema';
...
export const POST = withAuthTenant(
  ...
  const v = validateBody(body, rejectClaimSchema);
  if ('error' in v) return v.error;

  const reasonCode = String(v.data.reasonCode || '').trim().toUpperCase();
  const reasonText = String(v.data.reasonText || '').trim();
  ...
  const event = {
    ...
    rejectionReason: [reasonCode, reasonText].filter(Boolean).join(': ') || null,
    rejectedAt: now,
