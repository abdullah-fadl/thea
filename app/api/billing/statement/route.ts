...
export const GET = withAuthTenant(
  ...
  const catalogIds = Array.from(new Set(visibleEvents.map((item) => String(item.chargeCatalogId || '')).filter(Boolean)));
  const catalogs = catalogIds.length
    ? await prisma.billingChargeCatalog.findMany({
        where: { tenantId, id: { in: catalogIds } },
      })
    : [];
  const catalogById = catalogs.reduce<Record<string, (typeof catalogs)[number]>>((acc, catalog) => {
