// Phase 8.1.2 — FHIR R4 CoverageEligibilityRequest read endpoint (NPHIES eligibility, read-only).
// GET /api/fhir/CoverageEligibilityRequest/[id] → FhirCoverageEligibilityRequest
...
// CoverageEligibilityResponse route — request and response are two
  ...
  withErrorHandler(async (_req: NextRequest, { tenantId }, params) => {
    ...
    if (!id) {
      return NextResponse.json(
        operationOutcomeError('error', 'invalid', 'Missing CoverageEligibilityRequest id'),
    ...
    if (!log) {
      return NextResponse.json(
        notFoundOutcome('CoverageEligibilityRequest', id),
        { status: 404, headers: FHIR_HEADERS },
      );
    }
    const fhir = serializeCoverageEligibilityRequest(log, tenantId);
