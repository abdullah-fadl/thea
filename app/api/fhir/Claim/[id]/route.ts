// Phase 8.1.1 — FHIR R4 Claim read endpoint (NPHIES financial, read-only).
// GET /api/fhir/Claim/[id] → FhirClaim (NPHIES profile) or OperationOutcome 404.
...
export const GET = withAuthTenant(
  withErrorHandler(async (_req: NextRequest, { tenantId }, params) => {
    ...
    if (!claim) {
      return NextResponse.json(notFoundOutcome('Claim', id), { status: 404, headers: FHIR_HEADERS });
    }
    const fhir = await serializeClaim(claim, tenantId);
