...
export async function GET(
  ...
  try {
    ...
    // Get candidate details (tenant scoping enforced via tenantId filter).
    const { db } = await getPlatformClient();
    const candidatesCollection = db.collection('cvision_candidates');
...
export async function POST(
  ...
  try {
    ...
    // Get candidate (tenant scoping enforced via tenantId filter).
    const { db } = await getPlatformClient();
    const candidatesCollection = db.collection('cvision_candidates');
    ...
    // NOTE: cvision_offer_tokens has no Prisma model yet — this is a no-op
    // until a CvisionOfferToken model is introduced. Behaviour is safe (route
    // returns 404 on stale tokens because validateOfferToken also no-ops).
    const tokensCollection = db.collection('cvision_offer_tokens');
