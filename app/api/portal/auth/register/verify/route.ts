...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  // --- Emit patient.registered@v1 (best-effort, fire-and-forget for failures) ---
  // Fires only when a brand-new portal user row was inserted on this request.
  if (isFreshlyCreated) {
    try {
      await emit({
      ...
      logger.error('events.emit_failed', { category: 'portal', eventName: 'patient.registered', error: e });
    }
  }
