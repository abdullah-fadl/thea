...
export const GET = withErrorHandler(async (request: NextRequest) => {
  // When slug routing is active, this endpoint is retired.
  if (isEnabled('FF_PORTAL_SLUG_ROUTING')) {
    return NextResponse.json(
      {
        gone: true,
        message: 'Use /api/portal/tenant/:slug and direct patients to /p/:tenantSlug instead.',
      },
      { status: 410 },
    );
  }
