...
const grantPrivilegeSchema = z.object({
...
export const POST = withAuthTenant(async (req, { user, tenantId }) => {
  try {
    ...
    const v = validateBody(body, grantPrivilegeSchema);
    ...
    logger.error('Grant privilege error', { category: 'api', route: 'POST /api/admin/privileges/grant', error });
