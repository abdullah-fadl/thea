...
describe('FHIR — Route Guards', () => {
  ...
  // FHIR-18: Patient route uses withAuthTenant
  it('FHIR-18: Patient route uses withAuthTenant and withErrorHandler', () => {
    ...
    expect(src).toContain('fhirSearch')
    expect(src).toContain('fhirCreate')
    expect(src).toContain("permissionKey: 'opd.visit.view'")
  })
})
