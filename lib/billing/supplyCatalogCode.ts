...
export async function allocateSupplyCatalogCode(args: { db?: any; tenantId: string }) {
  const tenantUuid = await resolveTenantUuid(args.tenantId);
  const rows: any[] = await prisma.$queryRawUnsafe(
    `INSERT INTO "supply_catalog_counters" ("tenantId", "seq")
     VALUES ($1, 1)
     ON CONFLICT ("tenantId")
     DO UPDATE SET "seq" = "supply_catalog_counters"."seq" + 1
     RETURNING "seq"`,
    tenantUuid
  );
  const seq = Number(rows[0]?.seq || 1);
  return `SUP-${zeroPad(seq, 4)}`;
}
