...
export interface WithAuthTenantOptions {
  ...
  sanitize?: boolean;
  /** If true, injects user.hospitalId into context and enforces it when FF_HOSPITAL_SCOPED_GUARD is ON.
   *  When flag is OFF, this option is silently ignored (backward-compatible).
   *  When flag is ON and hospitalScoped=true: users without a hospitalId get 403 reason="no_hospital_scope". */
...
export type AuthTenantHandler = (
  ...
  context: {
    ...
    permissions: string[];
    role: string;
    /** Populated when hospitalScoped=true AND FF_HOSPITAL_SCOPED_GUARD is ON. Undefined otherwise. */
...
export function withAuthTenant(
  ...
  const wrappedHandler = async (
    ...
    // Hospital-scoped guard (FF_HOSPITAL_SCOPED_GUARD) — flag OFF = no-op
    let resolvedHospitalId: string | null | undefined;
