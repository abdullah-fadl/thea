      ...
      <div className="relative z-10 max-w-7xl mx-auto container-padding">
        ...
        >
          <span className="inline-block px-4 py-1.5 rounded-full bg-thea-primary/10 border border-thea-primary/20 text-thea-primary text-sm font-medium mb-4">
            {tr('حلولنا', 'Our Solutions')}
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white mb-4">
            {tr('منتجاتنا', 'Our Products')}
          </h2>
          <p className="text-slate-500 dark:text-white/50 text-lg max-w-2xl mx-auto">
            {tr('منصات مؤسسية مصممة للسوق السعودي، مدعومة بالذكاء الاصطناعي.', 'Enterprise-grade platforms designed for the Saudi market, powered by artificial intelligence.')}
          </p>
        </motion.div>
          ...
          {products.map((product) => {
            ...
            return (
              <motion.div key={product.id} variants={cardVariants} className="group relative">
                <div className={cn("absolute -inset-[1px] rounded-2xl bg-gradient-to-r opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm", product.gradient)} />
                <div className={cn("absolute -inset-[1px] rounded-2xl bg-gradient-to-r opacity-0 group-hover:opacity-50 transition-opacity duration-500", product.gradient)} />
                ...
                <div className="relative rounded-2xl bg-white dark:bg-thea-dark backdrop-blur-xl border border-slate-200 dark:border-white/10 p-8 h-full flex flex-col group-hover:border-transparent transition-colors duration-500 shadow-sm dark:shadow-none">
                  <div className={cn("w-14 h-14 rounded-xl bg-gradient-to-br flex items-center justify-center mb-6", product.gradient)}>
                    <MainIcon className="w-7 h-7 text-white" />
                  </div>

                  <h3 className={cn("text-2xl font-bold text-slate-900 dark:text-white mb-3", isArabic && "font-arabic")}>
                    {tr(product.titleAr, product.titleEn)}
                  </h3>

                  <p className={cn("text-slate-500 dark:text-white/50 leading-relaxed mb-6", isArabic && "font-arabic")}>
                    {tr(product.descriptionAr, product.descriptionEn)}
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-8 flex-1">
                    {product.features.map((feature, idx) => {
                      ...
                      return (
                        <div key={idx} className={cn("flex items-center gap-2.5 text-sm text-slate-600 dark:text-white/70", isArabic && "flex-row-reverse text-right font-arabic")}>
                          <FeatureIcon className="w-4 h-4 text-thea-primary shrink-0" />
                          <span>{tr(feature.ar, feature.en)}</span>
                        </div>
                      );
                    })}
                  </div>

                  <Link href={product.href} className={cn("inline-flex items-center gap-2 text-thea-primary font-semibold transition-all duration-300 group-hover:gap-3", isArabic && "flex-row-reverse font-arabic")}>
                    {tr('اعرف المزيد', 'Learn More')}
                    <ArrowRight className={cn("w-4 h-4 transition-transform duration-300 group-hover:translate-x-1", isArabic && "rotate-180 group-hover:-translate-x-1")} />
                  </Link>
                </div>
              </motion.div>
            );
